...
const testPollFieldsWithExpansionReturnsPollIds = <
    TParams =
...
>(
    ...
    const name =
        options.name ??
        `given list of pollFields and '${TweetExpansions.AttachmentsPollIds}', it returns tweets with list of poll_ids`;
    test(name, async () => {
        // Arrange
